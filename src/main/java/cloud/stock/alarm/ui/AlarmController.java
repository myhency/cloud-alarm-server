...
import cloud.stock.alarm.AnalyzedItemSaveRequest;
...
import cloud.stock.alarm.AnalyzedItemSaveResponse;
...
public class AlarmController {
    ...
    public AlarmController(AlarmRepository alarmRepository, ModelMapper modelMapper) {
    ...
    public ResponseEntity createAlarm(@RequestBody @Valid AnalyzedItemSaveRequest analyzedItemSaveRequest, BindingResult bindingResult) {
        ...
        Alarm alarm = modelMapper.map(analyzedItemSaveRequest, Alarm.class);
        Alarm newAlarm = this.alarmRepository.save(alarm);

        AnalyzedItemSaveResponse analyzedItemSaveResponse = new AnalyzedItemSaveResponse(
                true,
                "OK",
                newAlarm.getId(),
                newAlarm.getItemName(),
                newAlarm.getItemCode()
        );
